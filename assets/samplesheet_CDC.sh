...
if [ "$answer" == "n" ]; then
    ...
    # Function to rename files according to the specified pattern
    rename_files() {
        echo "Renaming files in directory: $data_dir"
        for folder in "$data_dir"/*; do
            if [[ -d "$folder" ]]; then
                for fastq_file in "$folder"/*.{fastq,fastq.gz}; do
                    if [[ -f "$fastq_file" ]]; then
                        fastq_name=$(basename "$fastq_file")
                        echo "Checking file: $fastq_name"
                        if [[ "$fastq_name" == EXP-PBC096_* ]]; then
                            new_name=$(echo "$fastq_name" | sed 's/^EXP-PBC096_//')
                            mv "$fastq_file" "$folder/$new_name"
                            echo "Renamed $fastq_name to $new_name in $folder"
                        fi
                    fi
                done
            fi
        ...
        if [[ -d "$folder" ]]; then
            for fastq_file in "$folder"/*.{fastq,fastq.gz}; do
                if [[ -f "$fastq_file" ]]; then
                    ...
                    # Compress and rename the file to .fastq.gz if it's not already compressed
                    if [[ "$fastq_file" == *.fastq ]]; then
                        ...
                        cp "$fastq_file" "$output_dir/$sample_name/${sample_name}.fastq.gz"
                        echo "Copied $fastq_file to ${sample_name}.fastq.gz at $(date '+%Y-%m-%d %H:%M:%S')"
                    fi
                fi
            done
        fi
    ...
    echo "Samplesheet created: $samplesheet at $(date '+%Y-%m-%d %H:%M:%
